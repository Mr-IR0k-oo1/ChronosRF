...
pub fn build_plan(
    ...
    let sequence = match scenario {
        AttackScenario::CoordinatedEmitter => vec![
            frame(base_time, frequency_start_hz, frequency_end_hz, bin_width_hz, sample_count, vec![-30.0, -80.0, -80.0], 400),
            frame(base_time + ChronoDuration::milliseconds(400), frequency_start_hz, frequency_end_hz, bin_width_hz, sample_count, vec![-80.0, -80.0, -80.0], 700),
            frame(base_time + ChronoDuration::milliseconds(1_100), frequency_start_hz, frequency_end_hz, bin_width_hz, sample_count, vec![-12.0, -80.0, -80.0], 400),
            frame(base_time + ChronoDuration::milliseconds(1_500), frequency_start_hz, frequency_end_hz, bin_width_hz, sample_count, vec![-80.0, -80.0, -80.0], 700),
            frame(base_time + ChronoDuration::milliseconds(2_200), frequency_start_hz, frequency_end_hz, bin_width_hz, sample_count, vec![-8.0, -80.0, -80.0], 400),
            frame(base_time + ChronoDuration::milliseconds(2_600), frequency_start_hz, frequency_end_hz, bin_width_hz, sample_count, vec![-80.0, -80.0, -80.0], 700),
            frame(base_time + ChronoDuration::milliseconds(3_300), frequency_start_hz, frequency_end_hz, bin_width_hz, sample_count, vec![-6.0, -80.0, -80.0], 0),
        ],
        AttackScenario::PersistentEmitter => vec![
            frame(base_time, frequency_start_hz, frequency_end_hz, bin_width_hz, sample_count, vec![-28.0, -80.0, -80.0], 600),
            frame(base_time + ChronoDuration::milliseconds(600), frequency_start_hz, frequency_end_hz, bin_width_hz, sample_count, vec![-18.0, -18.0, -80.0], 600),
            frame(base_time + ChronoDuration::milliseconds(1_200), frequency_start_hz, frequency_end_hz, bin_width_hz, sample_count, vec![-16.0, -16.0, -80.0], 600),
            frame(base_time + ChronoDuration::milliseconds(1_800), frequency_start_hz, frequency_end_hz, bin_width_hz, sample_count, vec![-14.0, -14.0, -80.0], 600),
            frame(base_time + ChronoDuration::milliseconds(2_400), frequency_start_hz, frequency_end_hz, bin_width_hz, sample_count, vec![-80.0, -80.0, -80.0], 0),
        ],
        AttackScenario::EscalatingBandActivity => vec![
            frame(base_time, frequency_start_hz, frequency_end_hz, bin_width_hz, sample_count, vec![-30.0, -80.0, -80.0], 400),
            frame(base_time + ChronoDuration::milliseconds(400), frequency_start_hz, frequency_end_hz, bin_width_hz, sample_count, vec![-80.0, -80.0, -80.0], 700),
            frame(base_time + ChronoDuration::milliseconds(1_100), frequency_start_hz, frequency_end_hz, bin_width_hz, sample_count, vec![-10.0, -12.0, -80.0], 400),
            frame(base_time + ChronoDuration::milliseconds(1_500), frequency_start_hz, frequency_end_hz, bin_width_hz, sample_count, vec![-80.0, -80.0, -80.0], 700),
            frame(base_time + ChronoDuration::milliseconds(2_200), frequency_start_hz, frequency_end_hz, bin_width_hz, sample_count, vec![-8.0, -8.0, -8.0], 0),
        ],
    };
...
mod tests {
    ...
    fn coordinated_emitter_plan_emits_parseable_sweeps() {
        ...
        for (index, frame) in plan.frames.iter().enumerate() {
            ...
            assert_eq!(fields.len(), 9, "frame {} should emit date, time, five sweep fields, and three powers", index + 1);
